...
##################################################
#```
#Zygalski Sheets Simulation - Choose Mode:
#1. Simplified Educational Version
#2. Historically Accurate Version
#3. FAST EDUCATIONAL 1
#4. FAST EDUCATIONAL 2
#Enter: 3
#Running simplified educational demo (fast example)...
#Rotor order: ['I', 'II', 'III']
#Testing a single left rotor position: 'A' with intercept ['1-4']

#Zygalski sheet for left rotor 'A' and female 1-4:
...
#Z  . . . . . . . . . . . . O . O . . . O . . . . O . .
#(Sheet for left rotor A)
#
#This demo is simplified to avoid heavy computation. It demonstrates the concept without extensive calculations.
#```
# -Thanks TO O1 pro mode
# -Thanks to GPT 4.5
# -Thanks to Rejewski for the idea :LenaDab:
##################################################
##################################################
##################################################
##################################################
...
def demo_educational_FAST_1():
    """
    A simplified educational demonstration with very limited iterations for fast execution.
    """
    print("Running simplified educational demo (fast example)...")
    rotor_order = ['I', 'II', 'III']
    intercepts = ["1-4"]  # Only one intercept for fast demo
    
    left_start = 'A'  # Single fixed position for very quick execution
    print(f"Rotor order: {rotor_order}")
    print(f"Testing a single left rotor position: '{left_start}' with intercept {intercepts}")
    
    # Generate just one sheet quickly
    sheet = generate_sheet(rotor_order, left_start, female_type=intercepts[0])
    
    # Quickly show result without stacking
    print(f"\nZygalski sheet for left rotor '{left_start}' and female {intercepts[0]}:")
    display_sheet(sheet, left_label=left_start)
    
    print("\nThis demo is simplified to avoid heavy computation. "
          "It demonstrates the concept without extensive calculations.")
...
def demo_educational_FAST_2():
    """
    Fast educational demonstration limiting combinations for quick execution.
    """
    print("Running FAST educational Zygalski sheet demo...")
    rotor_order = ['I', 'II', 'III']
    intercepts = ["1-4"]
    left_start = 'A'

    print(f"Rotor order: {rotor_order}")
    print(f"Testing left rotor position: '{left_start}' with intercept {intercepts}")

    # Limit brute-force range significantly for speed
    letters = ['A', 'M', 'Z']  # Minimal set for demo
    ...
    for mid_idx in [0, 12, 25]:  # Only three positions (A, M, Z)
        for right_idx in [0, 12, 25]:
            enigma = EnigmaMachine(
                [left_rotor, mid_rotor, right_rotor],
                reflector_wiring=REFLECTOR_B
            )
            start_pos = left_start + chr(mid_idx + ord('A')) + chr(right_idx + ord('A'))
            enigma.set_positions(start_pos)

            female_possible = False

            # Only test a very limited subset of keys
            for a in letters:
                for b in letters:
                    for c in letters:
                        key = a + b + c
    ...
    # Display simplified sheet
    print(f"\nSimplified Zygalski sheet for left rotor '{left_start}' (positions A, M, Z):")
    header = "     A M Z"
    print(header)
    for idx, label in zip([0, 12, 25], 'AMZ'):
        line = f"  {label}  "
        for col_idx in [0, 12, 25]:
            hole = sheet_matrix[idx][col_idx]
            line += 'O ' if hole else '. '
        print(line)

    print("\nThis optimized demo illustrates Zygalski sheet logic with minimal computation.")
...
def main():
    ...
    print("3. FAST EDUCATIONAL 1")
    print("4. FAST EDUCATIONAL 2")
    ...
    if choice == '1':
        ...
        demo_educational_FAST_1()
    elif choice == '4':
        demo_educational_FAST_2()
    else:
